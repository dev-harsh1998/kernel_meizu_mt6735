...
#endif
    s16 efusevalue[3];
    efusevalue[0] = (s16)pmic_Read_Efuse_HPOffset(0xa);
    efusevalue[1] = (s16)pmic_Read_Efuse_HPOffset(0xb);
    efusevalue[2] = (s16)pmic_Read_Efuse_HPOffset(0xc);
...
void EnableTrimbuffer(bool benable)
{
    if (benable == true)
    {
        Ana_Set_Reg(AUDDEC_ANA_CON4, 0x0001, 0x0001); //Enable HP & HS drivers bias circuit    //trim buffer enable bit MT6328
...
//        upmu_set_rg_vio18_cal(1);// for MT6328 E1 VIO18 patch only
        ...
        Ana_Set_Reg(AUDDEC_ANA_CON4, 0x1f01, 0xffff); //Enable HP & HS drivers bias circuit    //trim buffer enable bit MT6328
...
//        upmu_set_rg_vio18_cal(1);// for MT6328 E1 VIO18 patch only
        ...
        //Ana_Set_Reg(AUDDEC_ANA_CON4, 0x0700, 0xffff); //Enable HP & HS drivers bias circuit
//        Ana_Set_Reg(AUDDEC_ANA_CON4, 0x1f00, 0xffff); //Enable HP & HS drivers bias circuit    //trim
        Ana_Set_Reg(AUDDEC_ANA_CON4, 0x1f01, 0xffff); //Enable HP & HS drivers bias circuit    //trim
...
//        upmu_set_rg_vio18_cal(1);// for MT6328 E1 VIO18 patch only
        ...
        //set analog part (HP playback)
        Ana_Set_Reg(AUDNCP_CLKDIV_CON4, 0x0003, 0xffff); //Set NCP soft start mode as default mode
        ...
        Ana_Set_Reg(AUDDEC_ANA_CON6, 0x0841, 0xfeeb); //Enable cap-less HC LDO (1.5V) & LDO VA33REFGEN

 //       Ana_Set_Reg(AUDDEC_ANA_CON6, 0x0A41, 0xfeeb); //Enable cap-less HC LDO (1.5V) & LDO VA33REFGEN
 //       Ana_Set_Reg(AUDDEC_ANA_CON6, 0x2AC1, 0xfeeb); //Enable cap-less LC LDO (1.5V)
        Ana_Set_Reg(AUDDEC_ANA_CON6, 0x28C1, 0xfeeb); //Enable cap-less LC LDO (1.5V)
 
        Ana_Set_Reg(AUDDEC_ANA_CON7, 0x8000, 0x8000); //Enable NV regulator (-1.5V)
        Ana_Set_Reg(ZCD_CON0, 0x0000, 0xffff); //Disable AUD_ZCD
        
//        Ana_Set_Reg(AUDDEC_ANA_CON0, 0xE000, 0xffff); //Disable headphone, voice and short-ckt protection. HP and voice MUX is opened
        Ana_Set_Reg(AUDDEC_ANA_CON0, 0xE080, 0xffff); //Disable headphone, voice and short-ckt protection. HP and voice MUX is opened
        ...
        //        Ana_Set_Reg(AUDDEC_ANA_CON3, 0x0A00, 0x0f00); //Audio headphone speaker detection output mux selection:(10)HPR, Audio headphone speaker detection input mux enable:(01)DACLP
    }
    else
    {
        Ana_Set_Reg(AUDDEC_ANA_CON4, 0x0000, 0xffff); //Disable drivers bias circuit
...
static void Receiver_Speaker_Switch_Change(bool enable)
{
    ...
    if (enable)
    {
        printk("switch to receiver\n");
        ret = GetGPIO_Info(11, &pin_rcvspkswitch, &pin_mode_rcvspkswitch);
        if (ret < 0)
        {
            printk("Receiver_Speaker_Switch_Change GetGPIO_Info FAIL!!! \n");
            return;
        }
...
static int Receiver_Speaker_Switch_Get(struct snd_kcontrol *kcontrol,
                                       struct snd_ctl_elem_value *ucontrol)
{
    printk("%s() : %d\n", __func__,  mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_RECEIVER_SPEAKER_SWITCH]);
...
{
    printk("%s()\n", __func__);
    if ((ucontrol->value.integer.value[0]  == true) && (mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_RECEIVER_SPEAKER_SWITCH]  == false))
    {
        Receiver_Speaker_Switch_Change(true);
        mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_RECEIVER_SPEAKER_SWITCH] = ucontrol->value.integer.value[0];
    }
    else if ((ucontrol->value.integer.value[0]  == false) && (mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_RECEIVER_SPEAKER_SWITCH]  == true))
    {
        mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_RECEIVER_SPEAKER_SWITCH] = ucontrol->value.integer.value[0];
...
static const struct snd_kcontrol_new mt6331_snd_controls[] =
{
    ...
    SOC_ENUM_EXT("Receiver_Speaker_Switch", Audio_DL_Enum[11], Receiver_Speaker_Switch_Get, Receiver_Speaker_Switch_Set),
...
void SetMicPGAGain(void)
{
    ...
    printk("%s  AUDIO_ANALOG_VOLUME_MICAMP1 index =%d \n", __func__, index);
...
static bool GetAdcStatus(void)
{
    ...
    {
        if ((mCodec_data->mAudio_Ana_DevicePower[i] == true)&&(i != AUDIO_ANALOG_DEVICE_RECEIVER_SPEAKER_SWITCH))
        {
            return true;
        }
    }
...
//            Ana_Set_Reg(TOP_CKPDN_CON0, 0x0000, 0x2000); //bit[13] AUD_CK power down=0
            Ana_Set_Reg(TOP_CKPDN_CON0, 0x0000, 0x6000); //bit[14]+bit[13] AUD_CK power on=0 MT6328

            Ana_Set_Reg(AFE_DCCLK_CFG0, 0x2062, 0xffff); //dcclk_div=11'b00100000011, dcclk_ref_ck_sel=2'b00
...
//            Ana_Set_Reg(AUDENC_ANA_CON6, 0x1515, 0xffff); //default value
        ...
        if (ADCType == AUDIO_ANALOG_DEVICE_IN_ADC1) //main and headset mic
        {
            printk("%s  AUDIO_MICSOURCE_MUX_IN_1 mux =%d \n", __func__, mCodec_data->mAudio_Ana_Mux[AUDIO_MICSOURCE_MUX_IN_1]);
            if (mCodec_data->mAudio_Ana_Mux[AUDIO_MICSOURCE_MUX_IN_1] == 0) //"ADC1", main_mic
            {
                SetDCcoupleNP(AUDIO_MIC_BIAS0, mAudio_Analog_Mic1_mode); //micbias0 DCCopuleNP
                //Ana_Set_Reg(AUDENC_ANA_CON9, 0x0201, 0xff09); //Enable MICBIAS0, MISBIAS0 = 1P9V
                Ana_Set_Reg(AUDENC_ANA_CON9, 0x0701, 0xff09); //Enable MICBIAS0, MISBIAS0 = 2P5V
                if (ECMmode == 1) // differential
                {
//                    Ana_Set_Reg(AUDENC_ANA_CON9, 0x0217, 0xff1f); //Enable MICBIAS0 and MICBIAS1, MISBIAS0 = 1P9V, [6752] ALPS01824949
                    Ana_Set_Reg(AUDENC_ANA_CON9, 0x0717, 0xff1f); //Enable MICBIAS0 and MICBIAS1, MISBIAS0 = 2P5V, [6752] ALPS01824949
                    
                }
                else if (ECMmode == 2) //single end
                {
//                    Ana_Set_Reg(AUDENC_ANA_CON9, 0x0213, 0xff1f); //Enable MICBIAS0 and MICBIAS1, MISBIAS0 = 1P9V, [6752] ALPS01824949
                    Ana_Set_Reg(AUDENC_ANA_CON9, 0x0713, 0xff1f); //Enable MICBIAS0 and MICBIAS1, MISBIAS0 = 2P5V
                    
                }                
                else //MEMS
                {                    
//                    Ana_Set_Reg(AUDENC_ANA_CON9, 0x0211, 0xff1f); //Enable MICBIAS0 and MICBIAS1, MISBIAS0 = 1P9V , [6752] ALPS01824949         
                    Ana_Set_Reg(AUDENC_ANA_CON9, 0x0711, 0xff1f); //Enable MICBIAS0 and MICBIAS1, MISBIAS0 = 2P5V                                   
                }
...
//                Ana_Set_Reg(AUDENC_ANA_CON15, 0x0002, 0x000f); //Audio L PGA 12 dB gain
            ...
            else if (mCodec_data->mAudio_Ana_Mux[AUDIO_MICSOURCE_MUX_IN_1] == 1) //"ADC2", headset mic
            {
                ...
                if (ECMmode == 1) // differential
                {                
                    Ana_Set_Reg(AUDENC_ANA_CON9, 0x0770, 0xfff0); //Enable MICBIAS1, SwithP/N MISBIAS1 = 2P5V
                }
                else if (ECMmode == 2) //single end
                {
                    Ana_Set_Reg(AUDENC_ANA_CON9, 0x0730, 0xfff0); //Enable MICBIAS1 and MICBIAS1, MISBIAS0 = 1P9V, [6752] ALPS01824949
                }                
                else //MEMS
                {
//                    Ana_Set_Reg(AUDENC_ANA_CON9, 0x0710, 0xfff0); //MICBIAS1 DCC  on //DCC
                    Ana_Set_Reg(AUDENC_ANA_CON9, 0x0730, 0xfff0); //MICBIAS1 DCC  on //DCC, bit 5 to 1 to match ACCDET
        ...
        else if (ADCType == AUDIO_ANALOG_DEVICE_IN_ADC2) //ref mic
        {
            ...
            {                
//                Ana_Set_Reg(AUDENC_ANA_CON9, 0x0217, 0xff1f); //Enable MICBIAS0 and MICBIAS1, MISBIAS0 = 1P9V , [6752] ALPS01824949
                Ana_Set_Reg(AUDENC_ANA_CON9, 0x0717, 0xff1f); //Enable MICBIAS0 and MICBIAS1, MISBIAS0 = 2P5V
...
//            Ana_Set_Reg(AUDENC_ANA_CON15, 0x0020, 0x00f0); //Audio R PGA 12 dB gain
        ...
        Ana_Set_Reg(AUDENC_ANA_CON3, 0x0800, 0x0900); //PGA stb enhance
...
static bool TurnOnADcPowerDCCECM(int ADCType, bool enable)
{
    ...
    TurnOnADcPowerDCC(ADCType, enable, 1);
    return true;
}
#endif
...
static void mt6331_codec_init_reg(struct snd_soc_codec *codec)
{
    ...
    Ana_Set_Reg(AUDENC_ANA_CON9,  0x0000 , 0x0010); //power off mic bias1    
}
...
static int mt6331_codec_probe(struct snd_soc_codec *codec)
{
    ...
    pin_mode_extspkamp = pin_mode_extspkamp_2 = pin_mode_vowclk = pin_mode_audmiso = pin_mode_rcvspkswitch = 0;
