...
#ifdef CONFIG_ARCH_MT6735M
/* make sure all DVFS OPP use post-div = 2 */
#define PLL_DIV1_FREQ		(1261000)	/* KHz */
#else
...
static void _mt_cpufreq_set_cur_freq(struct mt_cpu_dvfs *p, unsigned int cur_khz, unsigned int target_khz);
...
static int _mt_cpufreq_restore_default_volt(struct mt_cpu_dvfs *p)
{
    ...
    /* set volt */
#ifdef CONFIG_CPU_DVFS_TURBO_MODE
    ret = _mt_cpufreq_set_cur_volt_locked(p,
		   TURBO_MODE_VOLT(_mt_cpufreq_get_turbo_mode(p, cpu_dvfs_get_cur_freq(p)),
...
static int _mt_cpufreq_get_pwr_idx(struct mt_cpu_dvfs *p, unsigned int freq, unsigned int ncpu)
{
    ...
    for (i = 0; i < p->nr_opp_tbl * possible_cpu; i++) {
        if (p->power_tbl[i].cpufreq_ncpu == ncpu
                && p->power_tbl[i].cpufreq_khz  == freq
           )
            return i;
    }
...
static void _kick_PBM_by_cpu(struct mt_cpu_dvfs *p)
{
    ...
    if (idx == -1) {  // Not found in power table
        ...
        cpufreq_dbg("@%s: cur_freq = %d, cur_volt = %d, cur_ncpu = %d, limited_power = %d\n",
...
void mt_cpufreq_set_power_limit_by_pbm(unsigned int limited_power)
{
	...
	if (p->limited_power_by_thermal != 0 && limited_power > p->limited_power_by_thermal) {
		cpufreq_dbg("@%s: power is limited by thermal(%d), ignore budget from PBM(%d)...\n",
			__func__, p->limited_power_by_thermal, limited_power);
		return;
	}

	if (limited_power == p->limited_power_by_pbm) {
		cpufreq_dbg("@%s: limited_power(%d mW) not changed, skip it!\n", __func__, limited_power);
		return;
	}
	...
	cpufreq_dbg("@%s: limited power from PBM = %d\n", __func__, p->limited_power_by_pbm);
...
static unsigned int _mt_cpufreq_cpu_dds_calc(unsigned int khz) /* XXX: NOT OK FOR 1007.5MHz */
{
    unsigned int dds = 0;
...
#if !defined(CONFIG_ARCH_MT6735M) || defined(CONFIG_CPU_DVFS_PERFORMANCE_TEST)
static void _mt_cpufreq_dfs_by_set_armpll(struct mt_cpu_dvfs *p, unsigned int dds, unsigned int ckdiv_sel)
{
 ...
 static void _mt_cpufreq_set_cur_freq(struct mt_cpu_dvfs *p, unsigned int cur_khz, unsigned int target_khz)
{
    ...
    cpufreq_dbg("@%s():%d, cur_khz = %d, target_khz = %d, dds = 0x%x, sel = %d\n",
                __func__,
                __LINE__,
                cur_khz,
                target_khz,
    ...
    //BUG_ON((dds & _BITMASK_(26 : 24)) != (1 << 24));     /* posdiv should be 2 */
...
static unsigned int _mt_cpufreq_calc_pmic_settle_time(unsigned int old_vproc, unsigned int old_vsram, unsigned int new_vproc, unsigned int new_vsram)
{
    unsigned delay = 100;
...
static int _mt_cpufreq_set_cur_volt(struct mt_cpu_dvfs *p, unsigned int volt) /* volt: vproc (mv*100) */
{
    ...
    if (volt > cur_volt)
        udelay(_mt_cpufreq_calc_pmic_settle_time(cur_volt, 0, volt, 0));   // vsram is autotracking, bypass it
...
static int _mt_cpufreq_set_cur_volt_extbuck(struct mt_cpu_dvfs *p, unsigned int volt) /* volt: vproc (mv*100) */
{
    ...
    if (volt > cur_vproc) {
        ...
        do {
            ...
            delay_us = _mt_cpufreq_calc_pmic_settle_time(old_vproc, old_vsram, cur_vproc, cur_vsram);
            udelay(delay_us);
...
static unsigned int _mt_cpufreq_search_available_volt(struct mt_cpu_dvfs *p, unsigned int target_khz)
{
    int i;
...
static int _mt_cpufreq_set_locked(struct mt_cpu_dvfs *p, unsigned int cur_khz, unsigned int target_khz, struct cpufreq_policy *policy)
{
...
static int _mt_cpufreq_get_idx_by_freq(struct mt_cpu_dvfs *p, unsigned int target_khz, unsigned int relation) /* return -1 (not found) */
{
    int new_opp_idx = -1;
    int i;
...
static int _mt_cpufreq_power_limited_verify(struct mt_cpu_dvfs *p, int new_opp_idx)
{
    ...
    i = _mt_cpufreq_get_idx_by_freq(p, target_khz, CPUFREQ_RELATION_H); // TODO: refine this function for idx searching
...
static unsigned int _mt_cpufreq_calc_new_opp_idx(struct mt_cpu_dvfs *p, int new_opp_idx)
{
    ...
    if (p->dvfs_disable_by_early_suspend) {
        if (is_fix_freq_in_ES) {
            ...
            idx = _mt_cpufreq_get_idx_by_freq(p, CPUFREQ_FIX_FREQ_FOR_ES, CPUFREQ_RELATION_L);
            if (idx != -1)
                new_opp_idx = idx;
            else
    ...
    if (p->limited_max_freq_by_user) {
        idx = _mt_cpufreq_get_idx_by_freq(p, p->limited_max_freq_by_user, CPUFREQ_RELATION_H);
...
void mt_cpufreq_thermal_protect(unsigned int limited_power)
{
...
#ifdef CONFIG_CPU_FREQ
    {
        ...
        cpufreq_dbg("@%s: found = %d, limited_power_idx = %d, limited_max_freq = %d, limited_max_ncpu = %d\n",
...
#define DORMANT_MODE_VOLT   85000
        ...
        if (cpu_dvfs_is_extbuck_valid()) {
            ...
            cpufreq_info("@%s: cur_vsram = %dmV, cur_vproc = %dmV\n",
                    __func__, _mt_cpufreq_get_cur_vsram(p) / 100, (p->ops->get_cur_volt(p)) / 100);
        }
        else {
...
#ifdef  CONFIG_CPU_DVFS_POWER_THROTTLING
        register_battery_percent_notify(&_mt_cpufreq_power_throttle_bat_per_CB, BATTERY_PERCENT_PRIO_CPU_L);
        register_battery_oc_notify(&_mt_cpufreq_power_throttle_bat_oc_CB, BATTERY_OC_PRIO_CPU_L);
        register_low_battery_notify(&_mt_cpufreq_power_throttle_low_bat_CB, LOW_BATTERY_PRIO_CPU_L);
#endif
    }
	...
	mt_cpufreq_set_pmic_cmd(
		...
		IDX_DI_VSRAM_NORMAL,
		(pmic_get_register_value(PMIC_VSRAM_VOSEL_OFFSET) << 8) | pmic_get_register_value(PMIC_VSRAM_VOSEL_DELTA)
	);
#endif
...
static int _mt_cpufreq_suspend(struct device *dev)
{
    ...
    for_each_cpu_dvfs(i, p) {
        ...
        _mt_cpufreq_set(MT_CPU_DVFS_LITTLE, p->idx_normal_max_opp); // XXX: useless, decided @ _mt_cpufreq_calc_new_opp_idx()
    }
...
static ssize_t cpufreq_fix_freq_in_es_proc_write(struct file *file, const char __user *buffer, size_t count, loff_t *pos)
{
    ...
    if (sscanf(buf, "%d", &fix_freq_in_ES) == 1)
...
static ssize_t cpufreq_limited_by_pbm_proc_write(struct file *file, const char __user *buffer, size_t count, loff_t *pos)
{
    int limited_power;
...
static ssize_t cpufreq_limited_by_thermal_proc_write(struct file *file, const char __user *buffer, size_t count, loff_t *pos)
{
    int limited_power;
