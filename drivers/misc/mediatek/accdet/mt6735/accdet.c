...
kal_uint8 ts3a225e_connector_type=TS3A225E_CONNECTOR_NONE;
...
int accdet_irq_handler(void)
{
	...
	if((pmic_pwrap_read(ACCDET_IRQ_STS) & IRQ_STATUS_BIT)&&((pmic_pwrap_read(ACCDET_IRQ_STS) & EINT_IRQ_STATUS_BIT)!=EINT_IRQ_STATUS_BIT)) {
		...
		ACCDET_DEBUG("CUST_EINT_ACCDET_TYPE = %x\n", CUST_EINT_ACCDET_TYPE);	
		if(cur_eint_state ==  EINT_PIN_PLUG_IN ) {
...
{ 
   ...
   /**************************************************************************************************/
    ...
    //mt_set_gpio_out(GPIO_FSA8049_PIN, GPIO_OUT_ONE);
#endif
    //mt_set_gpio_dir(GPIO_CAMERA_2_CMRST_PIN, GPIO_DIR_OUT);
    //mt_set_gpio_pull_enable(GPIO_CAMERA_2_CMRST_PIN, GPIO_PULL_DISABLE);
    //mt_set_gpio_out(GPIO_CAMERA_2_CMRST_PIN, GPIO_OUT_ZERO);
