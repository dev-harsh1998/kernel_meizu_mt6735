    ...
    )
{
    ...
    printk("[WiFi/HIF]HifInfo->HifRegBaseAddr=0x%p, HifInfo->McuRegBaseAddr=0x%p \n",
    ...
    if(IS_ERR(HifInfo->clk_wifi_dma)) {
        printk("[WiFi/HIF][CCF]cannot get HIF clk_wifi_dma clock.\n");
        return PTR_ERR(HifInfo->clk_wifi_dma);
    }
    printk("[WiFi/HIF][CCF]HIF clk_wifi_dma=0x%p\n", HifInfo->clk_wifi_dma);
#endif
#endif
