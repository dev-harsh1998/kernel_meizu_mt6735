...
#define Boot_Upgrade_Protect	//wenggaojian@wind-mobi.com 20150713
#define FTS_CTL_IIC
#define SYSFS_DEBUG
...
static void gesture_node_init(void);
static void gesture_node_uninit(void);

#define  KEY_GESTURE_U DOUBLE_TAP
#define  KEY_GESTURE_UP SWIPE_Y_UP
#define  KEY_GESTURE_DOWN SWIPE_Y_DOWN
#define  KEY_GESTURE_LEFT SWIPE_X_LEFT 
#define  KEY_GESTURE_RIGHT SWIPE_X_RIGHT
#define  KEY_GESTURE_O UNICODE_O
#define  KEY_GESTURE_E UNICODE_E
#define  KEY_GESTURE_M UNICODE_M 
#define  KEY_GESTURE_C UNICODE_C
#define  KEY_GESTURE_W UNICODE_W
#define  KEY_GESTURE_S UNICODE_S
#define  KEY_GESTURE_V UNICODE_V_UP
#define  KEY_GESTURE_Z UNICODE_Z
#define  KEY_GESTURE_ARROW_UP	UNICODE_V_UP
#define  KEY_GESTURE_ARROW_RIGHT	UNICODE_V_RIGHT
...
#define GESTURE_ARROW_RIGHT		0x52	
//#define GESTURE_ARROW_LEFT  0x51//ио?C?гд3a
#define GESTURE_ARROW_UP			0x53
//#define GESTURE_ARROW_DOWN		0x54//ио?V?идик?
...
struct Upgrade_Info fts_updateinfo[] =
{
	...
	{0x54,"FT5x46",TPD_MAXPOINTS_10,AUTO_CLB_NONEED,2, 2, 0x54, 0x2c, 10, 2000}, //wenggaojian@wind-mobi.com 20150713
};
				
struct Upgrade_Info fts_updateinfo_curr;
#define FTS_RESET_PIN	GPIO_CTP_RST_PIN

#define CTP_DIFF_WORK_FREQ 1
...
#ifdef CTP_DIFF_WORK_FREQ
static int ft5x06_work_with_ac_usb_plugin(int plugin);
int acc_usb_flag = 0;
int tpd_sleep = 0;
int ft5x06_init = 0;
#endif
...
void focaltech_get_upgrade_array(void)
{
	...
	u8 chip_id;
	u32 i;
/* wenggaojian@wind-mobi.com 20150713 begin */
#ifdef Boot_Upgrade_Protect
	chip_id = 0x54; //FT5x46 or FT5346 Chip_ID
#else
	i2c_smbus_read_i2c_block_data(i2c_client,FTS_REG_CHIP_ID,1,&chip_id);
#endif
/* wenggaojian@wind-mobi.com 20150713 end */
	printk("%s chip_id = %x\n", __func__, chip_id);
...
static void check_gesture(int gesture_id)
{
	printk("fts gesture_id==0x%x\n ",gesture_id);
		printk("[darren-ft] gesture_control_value = 0x%x\n",gesture_control_value);

		u32 report_data = 0;

  if(gesture_control_value == 0x01)
   {
	switch(gesture_id)
	{
		case GESTURE_LEFT:
			{
	  			printk("tengdq TP Gesture is KEY_GESTURE_LEFT");
				gesture_value = SWIPE_X_LEFT;
		...
		case GESTURE_RIGHT:
			{
	  			  printk("tengdq TP Gesture is KEY_GESTURE_RIGHT");
				gesture_value = SWIPE_X_RIGHT;
				input_report_key(tpd->dev, KEY_GESTURE, 1);
				input_sync(tpd->dev);
				input_report_key(tpd->dev, KEY_GESTURE, 0);
				input_sync(tpd->dev);			
		...
		case GESTURE_UP:
			{
	      printk("gesture_id TP Gesture is KEY_GESTURE_UP");

				gesture_value = SWIPE_Y_UP;
		...
		case GESTURE_DOWN:
			{
	      printk("gesture_id TP Gesture is KEY_GESTURE_DOWN");
				gesture_value = SWIPE_Y_DOWN;
				input_report_key(tpd->dev, KEY_GESTURE, 1);
				input_sync(tpd->dev);
				input_report_key(tpd->dev, KEY_GESTURE, 0);
				input_sync(tpd->dev);
		...
		case GESTURE_DOUBLECLICK:
			{
	      printk("gesture_id TP Gesture is KEY_GESTURE_DOUBLE_TAP");
				gesture_value = DOUBLE_TAP;
				input_report_key(tpd->dev, KEY_GESTURE, 1);
				input_sync(tpd->dev);
				input_report_key(tpd->dev, KEY_GESTURE, 0);
				input_sync(tpd->dev);
		...
		case GESTURE_O:
			{
				printk("tengdq TP Gesture is KEY_GESTURE_O");
				gesture_value = UNICODE_O;
				input_report_key(tpd->dev, KEY_GESTURE, 1);
				input_sync(tpd->dev);
				input_report_key(tpd->dev, KEY_GESTURE, 0);
				input_sync(tpd->dev);
		...
		case GESTURE_W:
			{
				printk("tengdq TP Gesture is KEY_GESTURE_W");
				gesture_value = UNICODE_W;
				input_report_key(tpd->dev, KEY_GESTURE, 1);
				input_sync(tpd->dev);
				input_report_key(tpd->dev, KEY_GESTURE, 0);
				input_sync(tpd->dev);
		...
		case GESTURE_M:
			{
	      printk("tengdq TP Gesture is KEY_GESTURE_M");
				gesture_value = UNICODE_M;
				input_report_key(tpd->dev, KEY_GESTURE, 1);
				input_sync(tpd->dev);
				input_report_key(tpd->dev, KEY_GESTURE, 0);
				input_sync(tpd->dev);
		...
		case GESTURE_E:
			{
	     	printk("tengdq TP Gesture is KEY_GESTURE_E");
				gesture_value = UNICODE_E;
				input_report_key(tpd->dev, KEY_GESTURE, 1);
				input_sync(tpd->dev);
				input_report_key(tpd->dev, KEY_GESTURE, 0);
				input_sync(tpd->dev);
		...
		case GESTURE_S:
			{
	      printk("tengdq TP Gesture is KEY_GESTURE_S");
				gesture_value = UNICODE_S;
				input_report_key(tpd->dev, KEY_GESTURE, 1);
				input_sync(tpd->dev);
				input_report_key(tpd->dev, KEY_GESTURE, 0);
				input_sync(tpd->dev);
		...
		case GESTURE_Z:
			{
	      printk("tengdq TP Gesture is KEY_GESTURE_Z");
				gesture_value = UNICODE_Z;
				input_report_key(tpd->dev, KEY_GESTURE, 1);
				input_sync(tpd->dev);
				input_report_key(tpd->dev, KEY_GESTURE, 0);
				input_sync(tpd->dev);
		...
		case GESTURE_C:
			{
	      printk("tengdq TP Gesture is KEY_GESTURE_C");
				gesture_value = UNICODE_C;
				input_report_key(tpd->dev, KEY_GESTURE, 1);
				input_sync(tpd->dev);
				input_report_key(tpd->dev, KEY_GESTURE, 0);
				input_sync(tpd->dev);
		...
		case GESTURE_ARROW_RIGHT:
			{
	    	printk("tengdq TP Gesture is KEY_GESTURE_>>>");
				gesture_value = UNICODE_V_L;
				input_report_key(tpd->dev, KEY_GESTURE, 1);
				input_sync(tpd->dev);
				input_report_key(tpd->dev, KEY_GESTURE, 0);
				input_sync(tpd->dev);
...
*/
   ...
   buf[0] = 0xd3;
	...
	if((pointnum * 4 + 8)<255)
		...
		{
			   ret = fts_i2c_Read(i2c_client, buf, 1, buf, 255);
			   ret = fts_i2c_Read(i2c_client, buf, 0, buf+255, 255);
			   ret = fts_i2c_Read(i2c_client, buf, 0, buf+255*2, (pointnum * 4 + 8)-255*2);
		}
		else if(765<(pointnum * 4 + 8) && (pointnum * 4 + 8)<1020)
		{
			   ret = fts_i2c_Read(i2c_client, buf, 1, buf, 255);
			   ret = fts_i2c_Read(i2c_client, buf, 0, buf+255, 255);
			   ret = fts_i2c_Read(i2c_client, buf, 0, buf+255*2, 255);
			   ret = fts_i2c_Read(i2c_client, buf, 0, buf+255*3, (pointnum * 4 + 8)-255*3);
		}
   if (ret < 0)
   {
	   printk( "%s read touchdata failed.\n", __func__);
...
/*dixiaobing@wind-mobi.com 20150603 end*/
    ...
    i2c_smbus_read_i2c_block_data(i2c_client, 0xd0, 1, &state);
		printk("tpd3 fts_read_Gestruedata state=%d\n",state);
...
#endif
#ifdef CTP_DIFF_WORK_FREQ
		ft5x06_work_with_ac_usb_plugin(acc_usb_flag);
#endif
	mt_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM);  
	msleep(30);
	tpd_halt = 0;
...
/*dixiaobing@wind-mobi.com 20150603 end*/
	...
	printk("fts-gesture suspend gesture_control_value = 0x%x\n",gesture_control_value);
			...
			memset(coordinate_y,0,255);
				...
				for(i=0;i<10;i++)
				{
				  ...
				  if(state==1)
				    ...
				    {
					  TPD_DMESG("TPD GESTURE WRITE 0x00\n");
					  fts_write_reg(i2c_client, 0xd0, 0x01);
					  msleep(10);
					  fts_read_reg(i2c_client, 0xd0, &state);
					 if(state==1)
	...
	/*			  if(i>=9)
				  {
				     TPD_DMESG("TPD GESTURE WRITE fail \n");
					 return;
					 } */
					 
						            					  
			i2c_smbus_read_i2c_block_data(i2c_client, 0xd0, 1, &state);
			printk("tpd fts_read_gesture ok data state=%d\n",state);
	       	 return;
	
	}
     		i2c_smbus_read_i2c_block_data(i2c_client, 0xd0, 1, &state);
			printk("tpd fts_read_gesture fail state=%d\n",state);
	#endif
...
static int ft5x06_work_with_ac_usb_plugin(int plugin)
{
	...
	return fts_write_reg(i2c_client, 0x8B, val);
}
...
{
	if (tpd_sleep == 0) {
		ft5x06_work_with_ac_usb_plugin(plugin);
	}
	acc_usb_flag = plugin;
...
#else
inline int ctp_work_with_ac_usb_plugin(bool plugin){ return 0;}
#endif
...
static DEVICE_ATTR(gesture_control, 0666, gesture_control_show, gesture_control_store);
static DEVICE_ATTR(gesture_data, 0666, gesture_data_show, NULL);
...
static struct kobject *gesture_kobj;
...
static void gesture_node_init(void)
{
	...
	gesture_kobj = kobject_create_and_add("mx_tsp", &devices_kset->kobj) ;
	if (gesture_kobj == NULL) {
		printk(KERN_ERR "[darren]%s: kobject_create_and_add failed\n", __func__);
		return;
	}
	
	ret = sysfs_create_group(gesture_kobj, gesture_attribute_group);
	if (ret < 0) {
...
{
	if(gesture_kobj){
		sysfs_remove_group(gesture_kobj, gesture_attribute_group);
		kobject_put(gesture_kobj);
	}	
}
