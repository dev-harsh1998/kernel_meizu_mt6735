...
static unsigned char CTPM_FW[] = {
	#include "Z160_M88_FT5436_Succes0x5f_LCD12_Ver0x02_20150713_app.i"    //wenggaojian@wind-mobi.com 20150713
};
...
static unsigned char ft5x46_ctpm_VidFWid_get_from_boot(  struct i2c_client *client )
{
	...
	for(i = 0;i < FTS_UPGRADE_LOOP; i++) 
	{
		...
		if (i_ret < 0)
		{
			FTS_DBG( "[FTS] Step 4: read vendor id from flash error when i2c write, i_ret = %d\n", i_ret);
			continue;
		}
		//fts_i2c_Read(client, auc_i2c_write_buf, 4, reg_val, 2);
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	u8 uc_tp_fm_ver,uc_tp_vendor_id,uc_boot_vendor_id;  //wenggaojian@wind-mobi.com 20150713
	int i_ret;
